...
static void handle_joystick(uint8_t mode, joystick_adc_t* adc, joystick_state_t* state, button_hold_t* btn_hold,
    ...
    if (mode == JOY_MODE_DIGITAL) { // DIGITAL mode
        ...
        if (sy < -JOY_DIGITAL_ON && !state->up) {
            register_code(keymap_key_to_keycode(layer, up));
            state->up = true;
        } else if (sy > -JOY_DIGITAL_OFF && state->up) {
            unregister_code(keymap_key_to_keycode(layer, up));
            state->up = false;
        }

        // DOWN
        if (sy > JOY_DIGITAL_ON && !state->down) {
            register_code(keymap_key_to_keycode(layer, down));
            state->down = true;
        } else if (sy < JOY_DIGITAL_OFF && state->down) {
            unregister_code(keymap_key_to_keycode(layer, down));
            state->down = false;
        }

        // LEFT
        if (sx < -JOY_DIGITAL_ON && !state->left) {
            register_code(keymap_key_to_keycode(layer, left));
            state->left = true;
        } else if (sx > -JOY_DIGITAL_OFF && state->left) {
            unregister_code(keymap_key_to_keycode(layer, left));
            state->left = false;
        }

        // RIGHT
        if (sx > JOY_DIGITAL_ON && !state->right) {
            register_code(keymap_key_to_keycode(layer, right));
            state->right = true;
        } else if (sx < JOY_DIGITAL_OFF && state->right) {
            unregister_code(keymap_key_to_keycode(layer, right));
            state->right = false;
        }

        // BUTTON
        if (adc->btn && !state->btn) {
            register_code(keymap_key_to_keycode(layer, btn));
            state->btn = true;
        } else if (!adc->btn && state->btn) {
            unregister_code(keymap_key_to_keycode(layer, btn));
            state->btn = false;
        }
